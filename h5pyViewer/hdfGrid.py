...
class DlgFormatSetup(wx.Dialog):
    def __init__(self, parent, fmt):
        ...
        self.cbPredef = cbPredef = wx.ComboBox(
            self, -1, choices=preDefLst, style=wx.CB_READONLY
        )
...
class HdfGridFrame(wx.Frame):
    def __init__(self, parent, lbl, hid):
        wx.Frame.__init__(
            self, parent, title="HDFGridView: " + lbl, size=wx.Size(750, 650)
        )
        ...
        if l == 1:
            ...
            for idx, l in enumerate(data.shape):
                ...
                sizer.Add(
                    wxAxCtrl.sizer, 0, wx.EXPAND | wx.ALIGN_CENTER | wx.ALL, border=5
                )
...
if __name__ == "__main__":
    ...
    def GetParser():
        ...
        exampleCmd = (
            (
                "compound n*m",
                "/scratch/detectorData/Ptychography/tst/initial_conditions_S01375.h5",
                "objects/object_0",
            ),
            (
                "compound n",
                "/scratch/detectorData/e14472/scan_00033.hdf5",
                "entry/data/spec",
            ),
            (
                "array l*m*n",
                "/scratch/detectorData/e14472/scan_00033.hdf5",
                "entry/data/pilatus_1",
            ),
            (
                "array n",
                "/scratch/detectorData/e14472/scan_00033.hdf5",
                "entry/data/pilatus_1_info",
            ),
        )

        epilog = "Examples:" + "".join(
            [
                "\n # " + s[0] + " #" + cmd + "--hdfFile %s --elem %s" % s[1:]
                for s in exampleCmd
            ]
        )
        ...
        parser.add_argument(
            "--elem", required=True, help="the path to the element in the hdf5 file"
        )
        return parser
