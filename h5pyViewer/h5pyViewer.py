...
from .hdfTree import HdfTreeCtrl
from .hdfGrid import HdfGridFrame
from .hdfAttrib import HdfAttribFrame, GetAttrVal
from .hdfImage import HdfImageFrame

try:
    from .hdfImageGL import HdfImageGLFrame
except ImportError as e:
    print(f"No GL image support. ImportError: {e}")
try:
    from .FrmPyFAI import HdfPyFAI1DFrame, HdfPyFAIFrame
except ImportError as e:
    print(f"No FAI image support. ImportError: {e}")
try:
    from .FrmProcRoiStat import ProcRoiStatFrame
except ImportError as e:
    print(f"No Roi stat support. ImportError: {e}")
...
class AboutFrame(wx.Frame):
    def __init__(self, parent):
        ...
        bmp = wx.StaticBitmap(
            panel,
            -1,
            wx.Bitmap(os.path.join(imgDir, "splash1.png"), wx.BITMAP_TYPE_ANY),
...
class HdfViewerFrame(wx.Frame):
    ...
    def OnOpen(self, event):
        dlg = wx.FileDialog(
            ...
            os.getcwd(),
            "",
            "HDF5 files (*.hdf5;*.h5)|*.hdf5;*.h5|all (*.*)|*.*",
            wx.FD_OPEN | wx.FD_CHANGE_DIR,
        )
        if dlg.ShowModal() == wx.ID_OK:
            path = dlg.GetPath()
    ...
    def GetPath(wxTree, wxNode):
        ...
        if wxTree.GetRootItem() == wxNodeParent:
            ...
            return (
                HdfViewerFrame.GetPath(wxTree, wxNodeParent)
                + "/"
                + wxTree.GetItemText(wxNode)
            )
    ...
    def GetPropertyStr(wxTree, wxNode):
        ...
        try:
            ...
            if numAttr > 20:
                ...
                for idxAttr in range(numAttr):
                    ...
                    txt += "\t" + str(aid.name) + "\t" + str(GetAttrVal(aid)) + "\n"
        val = None
        if t == h5py.h5g.GroupID:
            ...
            if (
                hid.shape == () or np.prod(hid.shape) < 10
            ):  # show up to max. 10 element arrays
