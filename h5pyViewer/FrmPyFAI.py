...
class MPLCanvasPyFAI1D(FigureCanvas):
    ...
    def InitChild(self, data):
        ...
        self.ai = pyFAI.AzimuthalIntegrator(
            1.0e3, ctrX, ctrY, 0.0, 0.0, 0.0, 1.0e0, 1.0e0
        )
        # canvas=self.canvas
...
class DlgSetupPyFAI(wx.Dialog):
    def __init__(self, parent):
        ...
        self.edCtrX = edCtrX = wx.TextCtrl(
            self, -1, "%g" % ai.get_poni1(), style=wx.TE_PROCESS_ENTER
        )
        self.edCtrY = edCtrY = wx.TextCtrl(
            self, -1, "%g" % ai.get_poni2(), style=wx.TE_PROCESS_ENTER
        )
        self.edNumPtTh = edNumPtTh = wx.TextCtrl(
            self, -1, "%g" % parent.numPtTh, style=wx.TE_PROCESS_ENTER
        )
        self.edNumPtCh = edNumPtCh = wx.TextCtrl(
            self, -1, "%g" % parent.numPtCh, style=wx.TE_PROCESS_ENTER
        )
        self.cbMethod = cbMethod = wx.ComboBox(
            self, -1, choices=("default", "numny"), style=wx.CB_READONLY
        )
