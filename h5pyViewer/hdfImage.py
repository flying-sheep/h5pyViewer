...
class MPLCanvasImg(FigureCanvas):
    def __init__(self, parent, SetStatusCB=None, fig=None):
        if SetStatusCB:
        ...
        self.fig = fig
        self.ax = ax  # type: matplotlib.axes.Axes
        self.colBar = None
    ...
    def InitChild(self, data):
        ...
        img = ax.imshow(
            data, interpolation="nearest", cmap=mpl.cm.viridis, vmin=vmin, vmax=vmax
        )
    ...
    def OnMotion(self, event):
        ...
        elif self.colBar and event.inaxes == self.colBar.ax:
            colBar = self.colBar
            pt = colBar.ax.bbox.get_points()[:, 1]
            nrm = colBar.norm
            vmin, vmax, p0, p1, pS = (nrm.vmin, nrm.vmax, pt[0], pt[1], event.y)
            if isinstance(
                colBar.norm, mpl.colors.LogNorm
            ):  # type(colBar.norm)==mpl.colors.LogNorm does not work...
                vS = 0
...
class DlgColBarSetup(wx.Dialog):
    def __init__(self, parent):
        ...
        self.edVMin = edVMin = wx.TextCtrl(
            self, -1, "%g" % nrm.vmin, style=wx.TE_PROCESS_ENTER
        )
        self.edVMax = edVMax = wx.TextCtrl(
            self, -1, "%g" % nrm.vmax, style=wx.TE_PROCESS_ENTER
        )

        txtTxrFunc = wx.StaticText(self, -1, "function")
        self.cbtxrFunc = cbtxrFunc = wx.ComboBox(
            self, -1, choices=("linear", "logarithmic"), style=wx.CB_READONLY
        )
        ...
        self.cbColMap = cbColMap = wx.ComboBox(
            self, -1, choices=colMapLst, style=wx.CB_READONLY
        )
...
class HdfImageFrame(wx.Frame):
    ...
    def BuildMenu(self, dtype):
        ...
        self.Bind(wx.EVT_MENU, self.OnInvertAxis, mnItem)
        mnItem = mn.Append(
            wx.ID_ANY, "Show Moments", "Show image moments ", kind=wx.ITEM_CHECK
        )
    ...
    def PlotMoments(self):
        ...
        l1 = a - b
        ang = (
            0.5 * np.arctan2(2 * u11, (u20 - u02)) / (2 * np.pi) * 360.0
        ...
        print(
            "xb:%g yb:%g cov:%g %g %g %g  ang:%g exc:%g"
...
if __name__ == "__main__":
    ...
    def GetParser(required=True):
        ...
        parser.add_argument(
            "--hdfFile", required=required, default=fnHDF, help="the hdf5 to show"
        )
        parser.add_argument(
            "--elem",
            required=required,
            default=elem,
            help="the path to the element in the hdf5 file",
        )
        return parser
