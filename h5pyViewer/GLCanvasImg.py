...
def MplAddColormap(m, lut):
    if type(lut) == dict:
        ...
        vRGB = list(
            zip(
                np.interp(kLst, kR, vR),
                np.interp(kLst, kG, vG),
                np.interp(kLst, kB, vB),
            )
        )
...
"""
    ...
    def UpdateImg(self):
        ...
        try:
            ...
            self.glImg = glImg = glumpy.image.Image(
                txrData, colormap=colMap, vmin=txrRng[0], vmax=txrRng[1]
            )
        else:
            if glImg.data.shape == self.data.shape:
                ...
                self.glImg = glImg = glumpy.image.Image(
                    txrData, colormap=colMap, vmin=glImg._vmin, vmax=glImg._vmax
                )
        glImg.update()
    ...
    def InitGL(self):
        ...
        self.UpdateImg()
        self.imgCoord = np.array(
            [-0.49, 0, 0.49, 1, -0.49, 0, 0.49, 1]
        )  # xmin,xmax,umin,umax,ymin,ymax,vmin,vmax
        pass
...
class DlgColBarSetup(wx.Dialog):
    def __init__(self, parent):
        ...
        self.edVMin = edVMin = wx.TextCtrl(
            self, -1, "%g" % dataRange[0], style=wx.TE_PROCESS_ENTER
        )
        self.edVMax = edVMax = wx.TextCtrl(
            self, -1, "%g" % dataRange[1], style=wx.TE_PROCESS_ENTER
        )
